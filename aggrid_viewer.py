...
from plot_creator import (
    get_pre_selected_idx,
    write_coins,
    write_coins_custom,
    write_bespoke_coins,
    create_comparison_df,
    load_images,
    gen_performance_ag_df,
    add_drawdown,
)
...
uniform_weights_dict = uniform_weights_dict(
    ids_with_histories[: st.session_state.max_coins]
)
...
strategy_dict = {
    "Uniform": uniform_weights_dict
}  # , 'Markowitz':markowitz_weights_dict}
...
def rerun_aggrid():
    ...
    st.session_state.performance_ag_df = gen_performance_ag_df(
...
if portfolio_type == "Create your own":
    ...
    """,
        ...
        bespoke_weights = write_coins_custom(
            names_with_histories[: st.session_state.max_coins]
        )
    ...
    non_zero_coins = [
        key
        for key in st.session_state.strategy_dict[portfolio_type].keys()
        if st.session_state.strategy_dict[portfolio_type][key] > 0
    ]
    with st.sidebar:
        st.markdown(portfolio_type + " portfolio weights (%):", unsafe_allow_html=False)
        write_coins(
            non_zero_coins,
            st.session_state.strategy_dict[portfolio_type],
            ids2names_dict,
        )
...
for asset in st.session_state.selected_assets:
    try:
        selected_indexes.append(
            list(st.session_state.performance_ag_df["Asset"]).index(asset)
        )
    except:
        pass
...
gb.configure_selection(
    "multiple", use_checkbox=True, pre_selected_rows=st.session_state.selected_indexes
)
gridOptions = gb.build()

st.subheader("Performance metrics")
grid_response = AgGrid(
    st.session_state.performance_ag_df,
    ...
    key=persist("aggrid"),
)  # MANUAL SELECTION_CHANGED MODEL_CHANGED, VALUE_CHANGED
