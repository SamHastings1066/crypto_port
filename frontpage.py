...
fig = go.Figure(
    data=[
        ...
        go.Scatter(
            x=df.index, y=df.MA20, line=dict(color="yellow", width=1), name="MA20"
        ),
        go.Scatter(
            x=df.index, y=df.MA50, line=dict(color="green", width=1), name="MA50"
        ),
        go.Scatter(
            x=df.index, y=df.MA100, line=dict(color="red", width=1), name="MA100"
        ),
    ]
)

fig.update_layout(
    go.Layout(xaxis={"showgrid": False}, yaxis={"showgrid": False}),
...
# Plotly line chart
fig = go.Figure()
fig.add_scattergl(x=df.index, y=df.Close, line={"color": "green"}, name="Up trend")
fig.add_scattergl(
    x=df.index,
    y=df.Close.where(df.Close <= df.Open[0]),
    line={"color": "red"},
    name="Down trend",
)
...
df_scrape["% Change"] = (
    df_scrape["% Change"].str.replace("%", "").str.replace(",", "").astype(float)
)
...
data = [
    go.Table(
        ...
        cells=dict(
            values=df_scrape[cols_to_show].values.T,
...
fig.update_layout(go.Layout(xaxis={"showgrid": False}, yaxis={"showgrid": False}))
st.plotly_chart(fig, use_container_width=True)
