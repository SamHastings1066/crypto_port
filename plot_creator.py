...
def create_comparison_df(all_returns_df, selected_assets):
    selected_assets_present = [
        asset for asset in selected_assets if asset in list(all_returns_df.columns)
    ]
    melt_df = pd.melt(
        all_returns_df, value_vars=selected_assets_present, ignore_index=False
    )
    melt_df.columns = ["Asset", "Value (USD)"]
    return melt_df
...
def gen_performance_df(all_returns_df, market_cap_dict, strategy_dict):
    ...
    performance_df["Type"] = [
        "Portfolio" if x in list(strategy_dict.keys()) else "Coin" for x in assets
    ]
...
def add_drawdown(fig, all_returns_df, selected_asset):
    ...
    fig.add_vrect(
        ...
        fillcolor="red",
        opacity=0.05,
        annotation_text=selected_asset + " maxdd",
    )
...
def write_coins(non_zero_coins, weights_dict, ids2names_dict, n_cols=2):
    ...
    for i, coin_id in enumerate(non_zero_coins):
        cols[i].slider(
            ids2names_dict[coin_id],
            min_value=0,
            max_value=100,
            value=int(weights_dict[coin_id] * 100),
            key=coin_id,
            disabled=True,
        )
...
def write_coins_custom(coin_names, n_cols=2):
    ...
    for i, coin_name in enumerate(coin_names):
        with cols[i]:
            weight = st_custom_slider(
                coin_name, min_value=0, max_value=100, value=50, key=coin_name
            )
        weights_list.append(weight)
    weights_list = [weight / sum(weights_list) for weight in weights_list]
    return weights_list
